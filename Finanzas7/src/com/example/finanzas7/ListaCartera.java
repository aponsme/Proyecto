...
public class ListaCartera extends ListActivity {
	List<Row> rows;
	Base base;
	SQLiteDatabase bd; 
	EditText edit;
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        while (fila.moveToNext()) {
        	...
        	row.setSubtitle(fila.getString(0));
            rows.add(row);  
        } 
...
//		rows.get(9).setChecked(true);
		...
		anadir.setOnClickListener(new OnClickListener()
		{
			...
			public void onClick(View v) {
				...
				alta();	
				Intent i=new Intent(ListaCartera.this, ListaCartera.class);
				startActivity(i);
				finish();
			}		
		});
		...
		getListView().setOnItemClickListener(new OnItemClickListener() {
			...
			Intent i=new Intent(ListaCartera.this, Seleccion.class);
			bd = base.getReadableDatabase();
			String[] columns ={"id"};
			Cursor fila = bd.query("carteras", columns, "nombre_cartera=?", new String[] { rows.get(position).getTitle() }, null, null, null);
	...
	public void baja() {
		...
		for(int i=0;i<rows.size();i++)
        {
        	...
        	{
				String[] columns ={"id"};
				Cursor fila = bd.query("carteras", columns, "nombre_cartera=?", new String[] { rows.get(i).getTitle() }, null, null, null);
        		//Cursor fila = bd.rawQuery("Select id from carteras where nombre_cartera="+rows.get(i).getTitle(), null);NO FUNCIONA
                while (fila.moveToNext()) {
                	aux=fila.getInt(0);
                }
